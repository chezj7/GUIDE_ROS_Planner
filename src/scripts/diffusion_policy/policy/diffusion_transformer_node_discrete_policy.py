...
class DiffusionTransformerNodeDiscretePolicy(BaseImagePolicy):
    def __init__(self, 
        ...
        print(f"DiffusionTransformerNodeDiscretePolicy: num_inference_steps={self.num_inference_steps}, ")
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        B, To = value.shape[:2]
        T = self.horizon
        print(f"predict_action: B={B}, T={T}, To={To}")
        ...
        t_end = time.time()
        # print(f"\n=== predict_action Timing ===")
        # print(f"Encoder (obs_encoder)       : {t_enc_end - t_enc_start:.4f}s")
        # print(f"Conditional sampling        : {t_sample_end - t_sample_start:.4f}s")
        # print(f"One-hot decode to real vals : {t_decode_end - t_decode_start:.4f}s")
        # print(f"Action slice (usable part)  : {t_slice_end - t_slice_start:.4f}s")
        # print(f"Total predict_action time   : {t_end - t_start:.4f}s")
        # print("=============================\n")
        return result
