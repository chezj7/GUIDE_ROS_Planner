...
class NodeManager:
    ...
    def add_node_to_dict_unknown(self, coords, local_frontiers, extended_local_map_info, robot_location):
        key = (coords[0], coords[1])
        node = Node(coords, local_frontiers, extended_local_map_info)
        node.is_unknown_node = 1
        # distance = np.linalg.norm(node.coords - robot_location)
        # MAX_UTILITY = 30
        # utility = -0.4 * distance + 35
        # utility = min(MAX_UTILITY, utility)
        # node.utility = int(round(utility))  # 四舍五入并转换为整数
        self.nodes_dict.insert(point=key, data=node)
        return node
    ...
    def remove_node_from_dict(self, node):
        ...
        self.nodes_dict.remove(list(node.coords))  # 如果列表中存的是 list 类型
    ...
    def remove_unconnected_nodes(self, origin):
        ...
        for node in not_connected_nodes:
            ...
            if not node.is_unknown_node:               
                self.removed_nodes.add(node)
                self.remove_node_from_dict(node)
            # else:
            #    print(f"[INFO] Preserving unknown node at {node.coords} (not connected)") 
    ...
    def get_rarefied_graph(self, robot_location, map_info):
        ...
        is_unknown_nodes = set()
        for node in self.nodes_dict.__iter__():
            if node.data.utility > 0 and node.data.is_unknown_node == 0:
                non_zero_utility_nodes.add(node.data)
            if node.data.is_unknown_node:
                is_unknown_nodes.add(node.data)
        ...
        key_current_node = KeyNode(current_node.coords, current_node.utility, current_node.visited,current_node.is_unknown_node)
        # print(f"[INFO] Key_current_node {key_current_node.is_unknown_node} (connected)")
        ...
        for node in nodes_in_cluster:
            if node.utility > 0 and node.is_unknown_node == 0:
                ...
                # print(f"[INFO] Path to node {node.coords} with utility {node.utility} has distance {dist}")
                    ...
                    if first_node:
                        ...
                        if coords not in self.nodes_dict.find(ref).data.neighbor_set and (
                            ...
                            if (key_node_coords[0], key_node_coords[1]) in self.key_node_dict.keys():
                                ...
                                key_node = KeyNode(node.coords, node.utility, node.visited,node.is_unknown_node)
        ...
        for center_node, dstar in self.cluster_center_node_dict.values():
            ...
            if dstar is not None:
                ...
                # print(" [INFO] A* path to cluster center", center_node.coords, "with distance", dist)
        ...
        # ensure all unknown nodes are in key_node_dict
        # print(f"[INFO] 当前 is_unknown_nodes 数量: {len(is_unknown_nodes)}")
        added = 0
        for node in is_unknown_nodes:
            coords = (node.coords[0], node.coords[1])
            if coords not in self.key_node_dict:
                key_node = KeyNode(node.coords, node.utility, node.visited, node.is_unknown_node)
                self.key_node_dict[coords] = key_node
                added += 1

        # print(f"[INFO] 新添加到 key_node_dict 的 KeyNode 数量: {added}")
...
class KeyNode:
    def __init__(self, coords, utility, visited,is_unknown):
        self.coords = coords
        self.utility = utility
        self.visited = visited
        self.neighbor_set = set()
        self.neighbor_set.add((self.coords[0], self.coords[1]))
        self.is_unknown_node = is_unknown 
